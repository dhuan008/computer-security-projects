...
temp1 = md5( (password+magic+salt).encode()).hexdigest()
print (temp1 + '\n')

alternate_sum = md5((password+salt+password).encode())
print ("alternate : " + alternate_sum.hexdigest() + '\n')

first_six = alternate_sum.digest()
#print ('temp: ' + first_six[0:6].decode('utf-16LE') )

# Input to intermediate0
print ('Input inter0: ' + ":".join("{:02x}".format(ord(c)) for c in password+magic+salt+first_six[0:6] + 'a\0\0'))

Intermediate0 = md5(( password+magic+salt+ first_six[0:6].decode('utf-16LE') + 'a\0\0').encode())
